...
if (WebUI.verifyTextPresent('Cliente con oferta en proceso', false)) {
    ...
    if (WebUI.click(findTestObject('Simulador_Punta/Page_Simulador/button_Ofertar'))) {
        WebUI.callTestCase(findTestCase('SP_Empaquetado - Copy'), [:], FailureHandling.STOP_ON_FAILURE)
        ...
        WebUI.callTestCase(findTestCase('SP_Unicredito - Copy'), [:], FailureHandling.STOP_ON_FAILURE)
    } else {
        WebUI.callTestCase(findTestCase('SP_Alia - Copy'), [:], FailureHandling.STOP_ON_FAILURE)
    }
